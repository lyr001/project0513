...
export class GetinputService {
  ...
  setinput(geo: any, placement: any, repeater: any) {
    ...
    /////phantom(还有三个类)
    console.log("To update", geo.phantom.value, "previous", this.a.phantom)
    this.a.phantom = this.setphantom(geo.phantom.value, this.a.phantom);
    /////system（还有三个类）
  ...
  switch_name(data: any, name: string, key: string, num: number) {
    ...
    if (this.a.phantom.name == name) { //phantom
      if (num == 2) {
        if (key == 'repeat') {
          this.a.phantom[key].push(null);
        } else if (key == 'translation') {
          this.a.phantom[key].push(new PlacementTranslation());
        } else if (key == 'rotation') {
          this.a.phantom[key].push(new PlacementRotation());
        }
      }
      if (this.a.phantom[key][num] == null) {
        this.a.phantom[key][num] = this.switchRepeat(data);
      }
      ...
      for (let j = 0; j < 4; j++) {
        if (this.a.scanner[Object.keys(this.a.scanner)[j]].name == name) {
          if (num == 2) {
            if (key == 'repeat') {
              this.a.scanner[Object.keys(this.a.scanner)[j]][key].push(null);
            } else if (key == 'translation') {
              this.a.scanner[Object.keys(this.a.scanner)[j]][key].push(new PlacementTranslation());
            } else if (key == 'rotation') {
              this.a.scanner[Object.keys(this.a.scanner)[j]][key].push(new PlacementRotation());
            }
          }
          if (this.a.scanner[Object.keys(this.a.scanner)[j]][key][num] == null) {
            this.a.scanner[Object.keys(this.a.scanner)[j]][key][num] = this.switchRepeat(data);
          }
      ...
      for (let n4 = 0; n4 < this.a.scanner.level4.length; n4++) {
        if (this.a.scanner.level4[n4].name == name) {
          if (num == 2) {
            if (key == 'repeat') {
              this.a.scanner.level4[n4][key].push(null);
            } else if (key == 'translation') {
              this.a.scanner.level4[n4][key].push(new PlacementTranslation());
            } else if (key == 'rotation') {
              this.a.scanner.level4[n4][key].push(new PlacementRotation());
            }
          }
          if (this.a.scanner.level4[n4][key][num] == null) {
            this.a.scanner.level4[n4][key][num] = this.switchRepeat(data);
          }
      ...
      for (let n5 = 0; n5 < this.a.scanner.level5.length; n5++) {
        if (this.a.scanner.level5[n5].name == name) {
          if (num == 2) {
            if (key == 'repeat') {
              this.a.scanner.level5[n5][key].push(null);
            } else if (key == 'translation') {
              this.a.scanner.level5[n5][key].push(new PlacementTranslation());
            } else if (key == 'rotation') {
              this.a.scanner.level5[n5][key].push(new PlacementRotation());
            }
          }
          if (this.a.scanner.level5[n5][key][num] == null) {
            this.a.scanner.level5[n5][key][num] = this.switchRepeat(data);
          }
  ...
  setphantom(pha: any, newpha: any) {
    if (Object.keys(pha[0].content).length < 10) {
      newpha = new Array(pha.length);
      for (let i = 0; i < pha.length; i++) {
        newpha[i] = new Volume();
        newpha[i] = this.setVolume(pha[i].content, newpha[i]);
      }
      return newpha;

    } else {
      newpha = new VoxelizedPhantom();
      newpha = this.assign(pha[0].content, newpha);
    }
    return newpha;
  }
    ...
    switch (Object.keys(shape).length) {
      case 1 : {
        if (Object.keys(shape)[0] == 'size') {
          newshape = new Box();
          return this.assign(shape, newshape);
        } else {
          newshape = new Tessellated();
          return this.assign(shape, newshape);
        }
      }
        ;
      ...
      case 3 : {
        switch (Object.keys(shape)[0]) {
          case 'size': {
            newshape = new Ellipsoid();
            return this.assign(shape, newshape);
          }
            ;
          case 'long': {
            newshape = new EllipticalTube();
            return this.assign(shape, newshape);
          }
            ;
          case 'path_to_ele_file': {
            newshape = new TetMeshBox();
            return this.assign(shape, newshape);
          }
        }
      }
  ...
  assign(item: any, newitem: any) {   //item 接受的表单数据
    for (let i = 0; i < Object.keys(newitem).length; i++) {
      ...
      if (Object.prototype.toString.call(item[Object.keys(item)[i]]) == '[object String]' ||
        ...
        newitem[Object.keys(newitem)[i]] = this.unit.vec3(item[Object.keys(item)[i]]);
      }//属性类型为Vec3（有单位的三维向量）
